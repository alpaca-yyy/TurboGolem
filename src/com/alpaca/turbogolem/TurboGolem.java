...
public class TurboGolem extends JavaPlugin implements Listener {
    ...
    private int boostGolem(LivingEntity golem) {
        ...
        for (Chest copperChest : copperChests) {
            // Skip copper chests occupied by a super golem (super golem handles its own)
            Block aboveChest = copperChest.getBlock().getRelative(BlockFace.UP);
            boolean occupiedBySuper = false;
            for (Entity e : world.getNearbyEntities(
                    aboveChest.getLocation().add(0.5, 0, 0.5), 0.8, 1.0, 0.8)) {
                if (e.getType() == copperGolemType && superGolems.contains(e.getUniqueId())) {
                    occupiedBySuper = true;
                    break;
                }
            }
            if (occupiedBySuper) continue;
            ...
            for (int slot = 0; slot < contents.length; slot++) {
                ...
                for (Container target : targets) {
                    ...
                    toMove.setAmount(Math.min(remaining, batchSize));
                    HashMap<Integer, ItemStack> overflow = dest.addItem(toMove);
            ...
            for (Location dest : touchedDestinations) {
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        world.spawnParticle(Particle.COMPOSTER, dest,
                                4, 0.5, 0.5, 0.5, 0);
                        tick += 5;
                    }
        ...
        for (int x = (int) box.getMinX(); x <= box.getMaxX(); x++) {
            for (int y = (int) box.getMinY(); y <= box.getMaxY(); y++) {
                for (int z = (int) box.getMinZ(); z <= box.getMaxZ(); z++) {
                    // Enforce a true horizontal CYLINDER (radius = radiusH, height = radiusV)
                    // so "/supergolem 12" literally means a 12-block-radius circle on the ground.
                    // Using a raw BoundingBox (cube) would over-reach to ~1.41x at the corners.
                    double dx = (x + 0.5) - center.getX();
                    double dz = (z + 0.5) - center.getZ();
                    double dy = (y + 0.5) - center.getY();
                    if (dx * dx + dz * dz > radiusH * radiusH) continue;
                    if (dy * dy > radiusV * radiusV) continue;
            ...
            switch (args[0].toLowerCase()) {
                ...
                case "killgolems" -> {
                    ...
                    for (UUID uuid : new HashSet<>(superGolems)) {
                        Entity e = Bukkit.getEntity(uuid);
                        if (e != null && e.isValid()) {
                            if (e instanceof LivingEntity le) le.setInvulnerable(false);
                            e.remove();
                            count++;
                        }
                        superGolems.remove(uuid);
